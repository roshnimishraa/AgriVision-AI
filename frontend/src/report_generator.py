"""
Builds a downloadable PDF summary of one analysis run: the photo (if used), the
detection result, and the yield forecast comparison. Uses fpdf2, which is pure-Python and
lightweight -- no system dependencies beyond what's already in requirements.txt.
"""
...
def build_report(cached):
    """`cached` is the same dict stored in st.session_state["analysis"] by
    src/ui/unified_page.py. Returns PDF bytes."""
    pdf = FPDF()
    pdf.add_page()
    ...
    if cached.get("has_photo") and cached.get("image") is not None:
        ...
        pdf.cell(0, 7, f"Severity score: {result['severity']:.2f} (0=healthy, 1=severe)", ln=True)
        pdf.ln(4)
    ...
    if cached.get("baseline_pred") is not None:
        pdf.cell(0, 7, f"If disease ignored: {cached['baseline_pred']:,.0f} hg/ha", ln=True)
    if cached.get("oracle_pred") is not None:
        pdf.cell(0, 7, f"Best-case (oracle) forecast: {cached['oracle_pred']:,.0f} hg/ha", ln=True)
